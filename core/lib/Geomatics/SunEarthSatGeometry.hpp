...
namespace gnsstk
{
      /**
       ...
       i.e. so that 'up' is perpendicular to the geoid, not from center of
       Earth, or geocentric coordinates, so that the vertical meets the center
       of the Earth. Return the vectors in the form of a 3x3 Matrix<double>,
       this is in fact the rotation matrix that will take an ECEF (XYZ) vector
       into 'north,east,up'. Individual unit vectors can be defined from this
   ...
   Matrix<double> NorthEastUpGeodetic(Position& P);
      ...
      /**
       ...
       East and North directions at that position. Use either geodetic
       coordinates, i.e. so that 'up' is perpendicular to the geoid, not from
       center of Earth, or geocentric coordinates, so that the vertical meets
       the center of the Earth. Return the vectors in the form of a 3x3
       Matrix<double>, this is in fact the rotation matrix that will take an
       ECEF (XYZ) vector into 'up,east,north'. Individual unit vectors can be
   ...
   Matrix<double> UpEastNorthGeodetic(Position& P);
      ...
      /**
       ...
       satellite, given the angular radii of both, and their angular separation.
       See code for more documentation, including derivations.
       ...
       @note  NB. units only need be consistent.
       @throw Exception
      */
   double ShadowFactor(double AngRadEarth, double AngRadSun,
                       double AngSeparation);

      /**
       Compute the fraction of the Sun covered by the earth as seen from a
       satellite, given Sun and satellite positions. Cf. the other version of
       this function.
   ...
   double ShadowFactor(const Position& SV, const Position& Sun);
      ...
      /**
       ...
       (ECEF) vectors X,Y,Z in the body frame of the satellite, namely
          ...
          the sun.
       Thus this rotation matrix R transforms an ECEF XYZ vector into the body
       frame of the satellite, so R * (ECEF XYZ vector) = components in body
       frame. Also, transpose(R) * (sat. body. frame vector) = ECEF XYZ
       components. Also return the shadow factor, which is the fraction of the
       sun's area not visible to satellite; thus sf == 1 means the satellite is
       in eclipse.
   ...
   Matrix<double> SatelliteAttitude(const Position& pos, const Position& Sun);
      ...
      /**
       ...
       velocity V, assuming an orbit-normal attitude. Return a 3x3 Matrix which
       contains, as rows, the unit (ECEF) vectors X,Y,Z in the body frame of the
       satellite, namely
          Z = along the boresight (i.e. towards Earth center),
          Y = perpendicular to orbital plane, pointing opposite the angular
          momentum vector X = along-track (same direction as velocity vector for
          circular orbit)
       Thus this rotation matrix R transforms an ECEF XYZ vector into the body
       frame of the satellite, so R * (ECEF XYZ vector) = components in body
       frame. Also, R.transpose() * (sat. body. frame vector) = ECEF XYZ
       components.
   ...
   Matrix<double> OrbitNormalAttitude(const Position& P, const Position& V);
      ...
      /**
       ...
       of receiver Position RX as seen at the satellite Position SV. The nadir
       angle is measured from the Z axis, which points to Earth center, and
   ...
   void SatelliteNadirAzimuthAngles(const Position& SV, const Position& RX,
                                    const Matrix<double>& Rot, double& nadir,
                                    double& azimuth);

      /**
       Compute the angle from satellite to Earth to Sun; that is the angular
       separation of the satellite and the Sun, as seen from the center of
       Earth. This angle lies between zero and pi, and it reaches zero (pi) only
       when the Sun lies exactly in the orbit plane at noon (midnight). NB. Use
       either class SolarSystem (high accuracy) or module SolarPosition (low
       accuracy) to get the Sun position. Return the angle in radians.
       @param SV Position        Satellite position
   ...
   double SatelliteEarthSunAngle(const Position& SV, const Position& Sun);
      ...
      /**
       ...
       satellite, given the satellite position and velocity and Sun position,
       all at one time. Return the angle in radians; it lies between +-PI/2 and
       has the sign of RxV. That is, the angle is positive if the Sun is out of
       the orbit plane in the direction of R cross V; then Sun "sees" the orbit
       motion as counter-clockwise. Also return phi, the angle, in the plane of
       the orbit, from midnight to the satellite; this lies between 0 and 2PI
       and increases in the direction of Vel. NB. Use either class SolarSystem
       (high accuracy) or module SolarPosition (lower accuracy) to get the Sun
       position.
       ...
       @note  NB. phi, beta and sesa, the satellite-earth-sun angle, form a
               right spherical triangle with sesa opposite the right angle. Thus
               cos(sesa)=cos(beta)*cos(phi).
       @throw Exception
      */
   void SunOrbitAngles(const Position& Pos, const Position& Vel,
                       const Position& Sun, double& beta, double& phi);

      /**
       Compute the nominal yaw angle of the satellite given the satellite
       position and velocity and the Sun position at the given time, plus a flag
       for GPS Block IIR and IIF satellites. Return the nominal yaw angle in
       radians, and the yaw rate in radians/second.
       ...
       @throw Exception
      */
   double SatelliteYawAngle(const Position& P, const Position& V,
                            const Position& Sun, bool blkIIRF,
                            double& yawrate);
