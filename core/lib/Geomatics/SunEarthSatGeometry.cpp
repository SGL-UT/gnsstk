...
namespace gnsstk
{
   ...
   Matrix<double> NorthEastUp(Position& P, bool geocentric)
   {
      try
      {
         ...
         double lat =
            (geocentric ? P.getGeocentricLatitude() : P.getGeodeticLatitude()) *
            DEG_TO_RAD;                              // rad N
   ...
   Matrix<double> NorthEastUpGeodetic(Position& P)
   {
      ...
      /* -----------------------------------------------------------------------------
         Given a Position, compute unit (ECEF) vectors in the topocentric Up, East,
   ...
   //------------------------------------------------------------------------------
      /* Consider the sun and the earth as seen from the satellite. Let the sun be
         ...
         overlap if |e-s| < R+r; complete overlap if |e-s| < R-r. The satellite is
         in penumbra if R-r < |e-s| < R+r, umbra if |e-s| < R-r.
         ...
         alpha and beta in terms of R, r and L using H and L relations above
         ...
         AngRadEarth    angular radius of the earth as seen at the satellite (any
         unit) AngRadSun      angular radius of the sun as seen at the satellite
         (same unit) AngSeparation  angular distance of the sun from the earth
         (same unit) return         fraction (0 <= f <= 1) of area of sun covered
         by earth units only need be consistent */
   ...
   double ShadowFactor(const Position& SV, const Position& Sun)
   {
      ...
      {
         Position z(SV);
            /* z points from satellite to Earth center - along the antenna
               boresight */
         ...
         ssun        = d * ssun; // unit vector Earth-to-Sun
            ...
            /* Sun-Earth-satellite angle = angular separation sat and Sun seen at
               earth same as SatelliteEarthSunAngle(SV,Sun) */
      ...
      /* -----------------------------------------------------------------------------
         Compute the satellite attitude, given the time and the satellite position
         SV. NB. Use either class SolarSystem (high accuracy) or module
         SolarPosition (low accuracy) to get the Sun position. Return a 3x3 Matrix
         that contains, as rows, the unit (ECEF) vectors X,Y,Z in the body frame of
         the satellite, namely
            Z = along the boresight (i.e. towards Earth center),
            Y = perpendicular to both Z and the satellite-sun direction, and
            X = completing the orthonormal triad. X will generally point toward the
            sun.
         Thus this rotation matrix R * (ECEF XYZ vector) = components in body
         frame, and transpose(R) * (sat. body. frame vector) = ECEF XYZ components.
   ...
   double SatelliteEarthSunAngle(const Position& Sat, const Position& Sun)
   {
      ...
      /* -----------------------------------------------------------------------------
         ...
         satellite. Return the angle in radians; it lies between +-pi/2 and has the
         sign of RxV. That is, the angle is positive if the Sun is out of the orbit
         plane in the direction of R cross V; then Sun "sees" the orbit motion as
         counter-clockwise. Also return, in phi, the angle, in the plane of the
         orbit, from midnight to the satellite; this lies between 0 and 2pi and
         increases in the direction of Vel. NB. Use either class SolarSystem (high
         accuracy) or module SolarPosition (lower accuracy) to get the Sun
         position.
         ...
         @param double beta    Return angle sun to plane of satellite orbit
         (radians)
         @param double phi     Return angle in orbit plane, midn to satellite
         (radians) NB. phi, beta and sesa - the satellite-earth-sun angle - form a
                       ...
                       const Position& Sun, double& beta, double& phi)
   {
      try
      {
         ...
         Position inertialV;
         inertialV.setECEF(SatV.X() - omega * SatR.Y(),
                           SatV.Y() + omega * SatR.X(), SatV.Z());
      ...
      /* -----------------------------------------------------------------------------
         ...
         @param Position V     Satellite velocity at time of interest (Cartesian,
         m/s)
         @param Position Sun   Sun position at time of interest
                            ...
                            double& yawrate)
   {
      try
      {
         ...
         double sinmu =
            ::sin(mu); // mu is the in-orbit-plane "azimuth" from midnite
         double cosmu =
            ::cos(mu); // increasing in direction of satellite motion
