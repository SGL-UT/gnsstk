   ...
   namespace experimental
   {
      ...
      add(const Vector<double>& Sol, const Matrix<double>& Cov)
      {
         try {
            ...
            // accumulate the info matrix
            sumInfo += Info;

            // accumulate the info vector
            // In Tapley (2004) (Figure 4.6.1, Batch processing algorithm flow chart) the info 
            // vector (N) is accumulated using H^T W y (where R^-1 = W)
            // Here we accumulate using Info * Sol3
            // Derivation showing how these are equivalent:
            // Info = H^T W H
            // Sol3 = x_hat = (H^T W H)^-1 H^T W y
            // Info * Sol3 = H^T W H (H^T W H)^-1 H^T W y
            //             = H^T W y
                     ...
                     Vector<double>& Slopes)
      {
         ...
         try
         {
            ...
            // start with solution = a priori, cut down to match current dimension
            ...
            try
            {
               ...
               bool tropSuccess;
               std::tie(tropSuccess, dX_norm, NIterations) =
            ...
            if (hasMemory)
            {
               // * Resids is effectively the vector of post-fit residuals, because it is the 
               // measurement residuals from the final iteration of iterativeSinglePointWLSSolution 
               // (= r_post)
               // * Solution is the final converged PRS solution (pos+clk) coming out of 
               // iterativeSinglePointWLSSolution for this epoch (= X_hat)
               // * localAPsol is the a priori PRS solution (pos+clk) for this epoch (= X_0)
               // * partials is the linearized partial derivative matrix of the measurements wrt the 
               // states, also coming out of iterativeSinglePointWLSSolution (= H)

               // Deriving how to compute the pre-fit residuals (= r_pre) from post-fit residuals
               ...
               // NOTE: now that we use PostFitResiduals instead of PreFitResiduals in addToMemory, I
               // believe we no longer need to compute these pre-fit residuals. However, I am leaving 
               // it in place 1) to ensure the safety of this legacy code, 2) because it's a relatively
               // cheap calculation, and 3) because they might be useful for other purposes in the 
               // future.
               
               // r_pre = y - h(X_0) (where y are the actual obs, and h is the non-linear measurement 
               // model)
               // r_post = y - h(X_hat)
               // Linearizing the measurement model h:
               // h(X_hat) \approx h(X_0) + H (X_hat - X_0)
               // r_post \approx y - [h(X_0) + H (X_hat - X_0)]
               // r_post \approx r_pre - H (x_hat - x_0)
               // r_pre \approx H (x_hat - x_0) + r_post
               PreFitResiduals = partials * (Solution - localAPSol) + Resids;
               LOG(DEBUG) << "Computed pre-fit residuals";
            }

            // save to member data
            PostFitResiduals = Resids;
            PostFitResiduals_RMS = RMS(Resids);
                  ...
                  NavSearchOrder order)
      {
         try
         {
            ...
            Vector<double> BestSol(3,0.0), BestPreFR, BestPostFR;
            ...
            for (int stage = 0;; ++stage)
            {
               ...
               do {
                  ...
                  if (BestRMS < 0.0 || PostFitResiduals_RMS < BestRMS)
                  {
                     BestRMS = PostFitResiduals_RMS;
                     BestSol = Solution;
                     BestSats = SatelliteIDs;
                     BestGNSS = dataGNSS;
                     BestSL = MaxSlope;
                     BestConv = dX_norm_final;
                     BestNIter = NIterations;
                     BestCov = Covariance;
                     BestInvMCov = invMeasCov;
                     BestPartials = Partials;
                     BestPreFR = PreFitResiduals;
                     BestPostFR = PostFitResiduals;
                     BestTropFlag = TropFlag;
                     BestIret = iret;
                  }

                  if (stage == 0 && PostFitResiduals_RMS < RMSLimit)
                  {
                     break;
                  }
            ...
            if (iret >= RETURN_CODE::OK)
            {
               ...
               PreFitResiduals = BestPreFR;
               PostFitResiduals = BestPostFR;
               dX_norm_final = BestConv;
               NIterations = BestNIter;
               PostFitResiduals_RMS = BestRMS;
               MaxSlope = BestSL;
               ...
               {
                  // accumulate the information matrix and vector using the input Solution and 
                  // Covariance, accumulate the chi-squared summation metric using the post-fit 
                  // residuals and inverse measurement covariance that is needed to compute final APV,
                  // accumulate the data count, and set the number of degrees of freedom
                  addToMemory(Solution, Covariance, PostFitResiduals, invMeasCov);
                  // update a priori solution
      ...
      outputRMSString(const std::string& tag, int iret)
      {
         ...
         oss << tag << " RMS " << printTime(currTime,gpsfmt)
            ...
            << std::fixed << std::setprecision(3)
            << " " << std::setw(8) << PostFitResiduals_RMS
            << std::setprecision(2)
            << " " << std::setw(7) << TDOP
            << " " << std::setw(7) << PDOP
            << " " << std::setw(7) << GDOP
                           ...
                           const Matrix<double>& weights) const
      {
         ...
         Matrix<double> info_matrix = partialsT * weights * partials;

         try
         {
            covariance = inverseSVD(info_matrix);
         }
                                 ...
                                 int niterLimit) const
      {
         ...
         // Initialize dX_norm used for convergence testing, and minimum number of iterations
         // defined outside of the loop so they can be returned
         double dX_norm = norm(dX);
         size_t iter = 2;

         for (; iter < niterLimit && dX_norm >= convLimit && dX_norm <= 1.e10; ++iter)
         {
            tropSuccess = singlePointWLSSolution(dX, partials, G, covariance, 
                                                residuals,
         ...
         return std::make_tuple(tropSuccess, dX_norm, iter);
      }
