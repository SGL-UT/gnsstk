...
namespace gnsstk
{
   ...
   {
      // int is the index into the original sats vector
      ...
      public:
         ...
         void setLabels(std::string label1, std::string label2, std::string label3)
      ...
      public:
         ...
         PRSolution2() : RMSLimit{6.5},
                        ...
                        hasMemory{true},
                        fixedAPriori{false},
                        nsol{0}, ndof{0}, ndata{0}, APV{0.0}, chiSquareSum{0.0},
                        Valid{false}
            {
               was.reset();
         ...
         /// RMS limit (m) on post-fit residuals RMS
         double RMSLimit;
         ...
         /// computation of pre-fit and post-fit residuals, and thus of the a posteriori variance of
         ...
         /// initialized with an a priori solution, which it 'remembers' from previous
         ...
         /// system clock of any missing system will not be part of the a priori state.
         bool hasMemory;
         ...
         /// Matrix<double> containing the measurement partial derivative matrix.
         ...
         /// Vector<double> of 'pre-fit' residuals
         /// Note: Only valid if memory exists b/c it needs a priori solution.
         /// Equal to Partials*(Sol-APrioriSol)+Resid, where Resid is the data residual vector from the
         /// final iteration of iterativeSinglePointWLSSolution, and thus equivalent to the post-fit 
         /// residual vector of the overall solution.
         /// Note: because of iteration that takes place in iterativeSinglePointWLSSolution, these
         /// residuals are not equal to the pre-fit residuals that are computed at the first 
         /// iteration within iterativeSinglePointWLSSolution, but rather the pre-fit residuals
         /// obtained by mapping the final post-fit residuals back through the linearization.
         Vector<double> PreFitResiduals;

         /// Vector<double> of 'post-fit' residuals
         /// Equal to the data residual vector from the final iteration of 
         /// iterativeSinglePointWLSSolution, and thus equivalent to the post-fit residual vector of 
         /// the overall solution.
         Vector<double> PostFitResiduals;

         /// Root mean square of the post-fit residuals (except when RMSDistanceFlag is set,
         /// then RMS distance from a priori position); in meters.
         double PostFitResiduals_RMS = std::numeric_limits<double>::quiet_NaN();
         ...
         /// the RSS change in solution at the end of iterations, initialize to infinity
         double dX_norm_final = std::numeric_limits<double>::infinity();
         ...
         ///                     in meters. On output, member dX_norm_final = final value.
         ///
         ...
         // ------------------------------------------------------------
         /// Fix the a priori solution to the given constant value (XYZ,m)
         /// and initialize the
         ...
         /// get the a posteriori variance of unit weight; return zero if not enough
         /// data has been collected.
         /// See "GNSS: Global Navigation Satellite Systems" by Hofmann-Wellenhof et al., section 
         /// 7.3.1, equation 7.78, page 240.
         /// And see Surveyor's Applications for Least Squares Adjustment (SALSA) User Manual, 
         /// section B.2.1 (p. 202).
         inline double getAPV(void) {
            if(ndof > 0) return chiSquareSum/ndof;
            return 0.0;
         }
         ...
         {
            try {
               // Output simple statistics on the full population of PRS (one for each epoch)
               was.setMessage(msg);
               os << was << std::endl << std::endl;

               Matrix<double> Cov(was.getCov());

               APV = getAPV();

               // scale covariance by APV
               if(ndof > 0)               
               {
                  Cov = Cov * APV;
               }
               ...
               // output APV and ndof
               os << "PRS " << msg << ": APV = "<< std::fixed << std::setprecision(3) << APV 
                  << " with " << ndof << " degrees of freedom." << std::endl;
               ...
               os << "PRS " << msg << ": Formal postfit covariance [meters^2], scaled by APV = " 
                  << std::endl << LM << std::endl;
               
            }
            catch(Exception& e) { GNSSTK_RETHROW(e); }
         }

         /// update a priori solution with a known solution; this is done at the end of RAIMCompute()
         void updateAPSolution(const Vector<double>& Sol)
         {
            ...
            // first call
            // NOTE: I don't think this code within the if statement is ever executed, as I don't think
            // APSolution.size() is ever zero, since updateAPSolution is called only at the end of 
            // RAIMCompute(). A grep search of `updateAPSolution` in both the gnsstk-apps repo and the 
            // gnsstk repo indicates no other use of this method.
            // However, I am leaving it in place 1) to ensure the safety of this legacy code, 
            // 2) because it's a relatively cheap calculation, and 3) because it might be useful in the 
            // future if this method is used to initialize the APSolution.
            ...
            // NOTE: This code is only necessary if Sol does not have the same number and type of 
            // clocks that are in APsolution, and I currently don't see how that would ever be the 
            // case. However, I am leaving it in place 1) to ensure the safety of this legacy code, 
            // 2) because it's a relatively cheap calculation, and 3) because it might be useful in the
            // future if Sol has different clocks than APsolution.
            // If future developers find that this is never necessary, recommend replacing this code 
            // with the much simpler APSolution = Sol; 
            // Initialize the temporary variable S to contain the position and all clock states
            Vector<double> S(3+allowedGNSS.size(),0.0);
            // Loop over position states
            for(i=0; i<3; i++)
               // If the a priori position is fixed, use that a priori value and not Sol
               // NOTE: fixedApriori does not appear to ever be true (the fixAPSolution method has a 
               // commented line "//fixedAPriori = true;   //TD user input", which is the only 
               // place I can see it might be set true, if the code is uncommented by a developer). 
               // However, I am leaving it in place 1) to ensure the safety of this legacy code, 
               // 2) because it's a relatively cheap calculation, and 3) because it might be useful in 
               // the future if the fixedAPriori flag is made into a user input option.
               S(i) = (fixedAPriori ? fixedAPrioriPos[i] : Sol(i));
            // Loop over clock states (one for each GNSS constellation)
            for(i=0; i<allowedGNSS.size(); i++) {
               // Use vectorindex from stl_helpers.hpp (core/lib/Utilities) to find the index of 
               // allowedGNSS[i] within the vector dataGNSS
               k = vectorindex(dataGNSS,allowedGNSS[i]);
               S(3+i) = (k == -1 ? APSolution[3+i] : Sol[3+k]);
            }
            
            // Set APSolution using the temporary variable S
            APSolution = S;
            
            // NOTE: this code was previously commented out at some point, but might be useful
            // to replace the above more complicated code - see NOTE above.
            //APSolution = Sol;
            //if(fixedAPriori)
         ...
         {
            // Add the solution and covariance to the was object, which accumulates the information
            // matrix and vector in order to provide an overall solution via the cumulative information
            // least squares approach
            was.add(Sol, Cov);

            try {

               // Create a new Matrix object invMC for the inverse measurement noise covariance, 
               // because if the input invMeasCov is empty, invMC is set to the identity matrix.
               // NOTE: from what I can tell, invMeasCov will never be empty when addToMemory is 
               // called (invMC.rows() == 0), but I'm leaving this code in place 1) to ensure the 
               // safety of this legacy code, 2) because it's a relatively cheap calculation, and 
               // 3) because it might be useful in the future if addToMemory *is* called with an empty 
               // invMeasCov. If a future developer finds that this code is used, please modify this 
               // comment to explain how/why. Otherwise perhaps just use the input invMeasCov instead 
               // of invMC.
               Matrix<double> invMC(invMeasCov);
               if(invMC.rows() == 0) {
                  invMC=Matrix<double>(PostFitResid.size(),PostFitResid.size());
                  ident(invMC);
               }

               // Compute the contribution to the chi-squared summation used to compute the APV, which
               // is equal to the post-fit residuals (PostFitResid) vector transposed times the inverse 
               // measurement noise covariance (invMC) matrix times PostFitResid
               // See "GNSS: Global Navigation Satellite Systems" by Hofmann-Wellenhof et al., section 
               // 7.3.1, equation 7.78, page 240.
               // And see Surveyor's Applications for Least Squares Adjustment (SALSA) User Manual, 
               // section B.2.1 (p. 202).
               chiSquareSum += dot(PostFitResid,invMC*PostFitResid);

               // Accumulate the data count
               ndata += PostFitResid.size();

               // Set the number of degrees of freedom each time addToMemory is called, so that it's 
               // ready for use in dumpSolution
               ndof = ndata - Sol.size();
               
            }
            catch(Exception& e) {
               e.addText("chiSquareSum failed.");
               GNSSTK_RETHROW(e);
            }
         }
      ...
      private:
         ...
         void computeSlopes(
             ...
             *    the vector of sats.
             * @param[in] eph NavLibrary to search ephemeris with
             * @param[in] order Specify whether to search by receiver behavior
             *    or by nearest in time.
