...
namespace gnsstk
{
   ...
   bool BDSD2NavAlm::isSameData(const NavDataPtr& right, bool ignore_timestamp) const
      {
         const std::shared_ptr<BDSD2NavAlm> alm = std::dynamic_pointer_cast<BDSD2NavAlm>(right);
         if (!alm)
         {
            return false;
         }

         return (BDSD2NavData::isSameData(right, true) &&
               (pnum == alm->pnum) &&
               (deltai == alm->deltai) &&
               (toa == alm->toa) &&
