...
class SEMTestFactory : public gnsstk::SEMNavDataFactory
{
public:
   gnsstk::NavValidityType getValidityFilter() const
   {
...
NavLibrary_T ::
    ...
    : ws(387, 589824.0, gnsstk::TimeSystem::GPS),
      toa(ws), ts(toa - 70 * 3600), te(toa + 74 * 3600)
{
}

const std::string fn("test_input_sem387.txt");

unsigned NavLibrary_T ::
    getXvtTest()
{
   TUDEF("NavLibrarySEM", "getXvt");
   gnsstk::Xvt xvt;
    ...
    getHealthTest()
{
   ...
   gnsstk::NavSatelliteID sat(10, 10, gnsstk::SatelliteSystem::GPS,
                              gnsstk::CarrierBand::L1, gnsstk::TrackingCode::CA,
                              gnsstk::NavType::GPSLNAV);
   TUASSERT(navLib.getHealth(sat, toa + 0, health, gnsstk::SVHealth::Any));
    ...
    epochTest()
{
   ...
   SEMTestFactory *rndfp = dynamic_cast<SEMTestFactory *>(ndfp.get());
   TUASSERT(rndfp->addDataSource(fname));
   gnsstk::CommonTime toa(gnsstk::GPSWeekSecond(gnsstk::SEMHeader::nearFullWeek, ws.sow, gnsstk::TimeSystem::GPS));
   std::cout << gnsstk::GPSWeekSecond(toa) << std::endl;
   TUASSERTE(gnsstk::CommonTime, toa - 70*3600, navLib.getInitialTime());
   gnsstk::SEMHeader::nearFullWeek = 0;
   TURETURN();
}
...
{
   TUDEF("NavLibrarySEM", "getAvailableSats");
