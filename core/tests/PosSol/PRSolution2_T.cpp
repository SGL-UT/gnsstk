...
unsigned PRSolution2_T::prSolutionDumpSolution()
{
   ...
   std::string expectedOutput = "Simple statistics on PRS\n"
                                ...
                                "\n"
                                "PRS PRS: APV = 0.000 with 0 degrees of freedom.\n"
                                "PRS PRS: Formal postfit covariance [meters^2], scaled by APV = \n"
...
raimComputeAPVTest()
{
   ...
   gnsstk::CommonTime start = startCivil.convertToCommonTime();

   gnsstk::CivilTime stopCivil(2025,05,26,0,0,30, TimeSystem::GPS);
   gnsstk::CommonTime stop = stopCivil.convertToCommonTime();
   
   // Load ephemeris data for day 146 (2025-05-26) and day 145 to handle
   // midnight cases where transmit time falls on previous day.
   std::string navfile = gnsstk::getPathData() + gnsstk::getFileSep() + "IGS0OPSFIN_20251460000_01D_15M_ORB.SP3";
   std::shared_ptr<NavDataFactory> navFactory = std::make_shared<SP3NavDataFactory>();
   navFactory->addDataSource(navfile);
   std::string navfilePrevDay = gnsstk::getPathData() + gnsstk::getFileSep() + "IGS0OPSFIN_20251450000_01D_15M_ORB.SP3";
   navFactory->addDataSource(navfilePrevDay);
   NavLibrary navLib;
   navLib.addFactory(navFactory);

   // Set reference position
   double refPosX = 4432980.6621;
   double refPosY = 3149432.093;
   ...
   std::vector<SatID> sats{
      SatID(5, SatelliteSystem::GPS),
      SatID(7, SatelliteSystem::GPS),
      SatID(12, SatelliteSystem::GPS),
      SatID(13, SatelliteSystem::GPS),
      SatID(14, SatelliteSystem::GPS),
      SatID(15, SatelliteSystem::GPS),
      SatID(17, SatelliteSystem::GPS),
      SatID(19, SatelliteSystem::GPS),
      SatID(22, SatelliteSystem::GPS),
      SatID(24, SatelliteSystem::GPS),
      SatID(30, SatelliteSystem::GPS)
   };
   ...
   // Verify that the RAIM algorithm works on the set of data at the first epoch
   Matrix<double> invMCov;
   int result = prsol.RAIMCompute(start, sats, pseudoranges, invMCov,
                                 navLib, &tropModel, NavSearchOrder::User);
   TUASSERTE(int, 0, result);
   TUASSERT(prsol.isValid());

   // Update pseudoranges with data from the second epoch
   pseudoranges.clear();
   pseudoranges = {
      24655042.893,
      24950564.931,
   ...
   // Verify that the RAIM algorithm works on the set of data at the second epoch
   Matrix<double> invMCov2;
   int result2 = prsol.RAIMCompute(stop, sats, pseudoranges, invMCov2,
                                 navLib, &tropModel, NavSearchOrder::User);
      
   TUASSERTE(int, 0, result2);
   TUASSERT(prsol.Valid);
      
   // Verify APV output
   std::ostringstream oss;
   prsol.dumpSolution(oss);
   std::string output = oss.str();
   std::string expected = "APV = 0.434 with 18 degrees of freedom.";
   TUASSERT(output.find(expected) != std::string::npos);
...
int main()
{
   ...
   errorTotal += testClass.raimComputeAPVTest();
