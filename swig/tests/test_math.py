...
def test_triple():
    ...
    a = gpstk.Triple(1, 2, 3)
    expected = ([1, 2, 3])
    actual = ([a[0], a[1], a[2]])
    check_test(actual, expected, "Triple Contructor and indexing check", test_num)
    test_num += 1
    # Conversion to stdvector and GPSTK vector
    a = gpstk.Triple(2, 4, 6)
    expected = ([2, 4, 6], [2, 4, 6])
    actual = (list(a.toVector()), list(a.toStdVector())) # Type is not quite correct
    check_test(actual, expected, "Conversion to stdvector and gpstk vector", test_num)
    test_num+=1
    # Cross and dot product
    a = gpstk.Triple(1.5, 2.3, 3.6)
    b = gpstk.Triple(3.4, 6.8, 4.5)
    np_a = np.array(gpstk.makeTuple(a))
    np_b = np.array(gpstk.makeTuple(b))
    expected = (np.dot(np_a, np_b), tuple(np.cross(np_a, np_b)))
    actual = (a.dot(b), gpstk.makeTuple(a.cross(b)))
    check_test(actual, expected, "Dot and Cross Product", test_num)
    test_num += 1
    # Magnitude Test
    a = gpstk.Triple(1.5, 2.3, 3.6)
    expected = np.linalg.norm(np.array(gpstk.makeTuple(a)))
    actual = a.mag()
    ...
    a = gpstk.Triple(2.3, 4.5, 3.6)
    b = gpstk.Triple(-9.0, 34.4, 23.2)
    actual = []
    c = a - b
    ...
    c = c + b
    actual.extend(round_list([c[0], c[1], c[2]]))
    actual.append(a == gpstk.Triple(2.3, 4.5, 3.6))
