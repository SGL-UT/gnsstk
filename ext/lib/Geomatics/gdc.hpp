...
namespace gnsstk
{
   ...
   public:
      ...
      static const unsigned
         BEG; ///<   1 beginning of data (beg-of-pass or after gap)
      ...
      static const unsigned
         REJ; ///< 128 segment of bad data - returnMessage() only
      ...
      class Arcinfo
      {
       public:
         ...
         // member data
         double
            step; ///< for slips, estimate of slip magnitude (data units).
      ...
      unsigned
         mark;   ///< bitmap identifying actions that created or modified Arc
   ...
   public:
      ...
      bool setParameter(std::string label, double value);
         ...
         */
      double getParameter(std::string label) { return CFG[label]; }
      ...
      int getUniqueNumber() { return unique; }
         ...
         */
      void ForceUniqueNumber(int n) { unique = n; }
      ...
      //---------------------------------------------------------------------------
         /**
          ...
          @throw Exception
         */
      int DiscontinuityCorrector(
         const RinexSatID& sat, const double& nominalDT, const Epoch& beginTime,
         std::vector<double> dataL1, std::vector<double> dataL2,
         std::vector<double> dataP1, std::vector<double> dataP2,
         std::vector<double> dt, std::vector<int> flags, std::string& retMsg,
   ...
   protected:
      ...
      static const unsigned
         WLOUTLIER; ///< flag for data called outlier by WL filter
      static const unsigned
         GFOUTLIER; ///< flag for data called outlier by GF filter
      ...
      static const unsigned
         ISOLATED; ///< flag for isolated good data (< MinPts)
      // others?
      ...
      int unique;
      std::string
         tag; ///< begin each output line with 'GDC \<unique\>' ( \<label\>)
      ...
      long long
         N1bias; ///< bias in L1 at initial point (from P1=wl1*(N1bias+L1))
      long long
         N2bias; ///< bias in L2 at initial point (from P2=wl2*(N2bias+L2))
      ...
      int filterFirstDiff(const unsigned which, const std::string label,
                          double limit, std::vector<FilterHit<double>>& hits);
      ...
      void getArcStats(const unsigned which)
      {
         try
         {
            for (std::map<int, Arc>::iterator ait = Arcs.begin();
                 ait != Arcs.end(); ++ait)
      ...
      void findArc(const unsigned int ind, std::map<int, Arc>::iterator& ait)
      {
         ...
         while (ait != Arcs.end() &&
                ind >= ait->second.index + ait->second.npts)
            ++ait;
         ...
         /**
          ...
          #GDC  1 GFW Arc2[772] WLslip/GFslip/WLfix/GFfix good/tot=438/445=98.43%
                              ...
                              -0.71 +-0.32
         */
      void DumpArcs(const std::string& tag, const std::string& label,
                    int prec = -1);
