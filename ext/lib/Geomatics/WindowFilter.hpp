...
namespace gpstk
{
   ...
   private:
      ...
      unsigned int
         halfwidth; ///< number of points on either side of slip analyzed
      ...
      const std::vector<T>
         &xdata; ///< reference to x-data to be filtered (TwoSample)
...
#undef inc
   ...
   void WindowFilter<T>::getStats(FilterHit<T>& sg, bool skip)
   {
      ...
      sg.mad =
         gpstk::Robust::MedianAbsoluteDeviation<T>(&sdv[0], i, sg.med, false);
      sg.haveStats = true;
      sdv.clear();
   }
