...
namespace gpstk
{
   ...
   public:
      ...
      class Analysis
      {
       public:
         ...
         // member data
         unsigned int
            index;     ///< index in original arrays to which this info applies.
         ...
         */
      FirstDiffFilter(const std::vector<T>& x, const std::vector<T>& d,
      ...
      inline T getLimit() { return fdlimit; }
      ...
      const std::vector<int>
         &flags;  ///< reference to flags, parallel to data, 0 == good
   ...
   private:
         /**
          ...
          @param dumpmsg string for analysis dump when bool dump is true
         */
      int analyze2(double ratlim, double siglim, bool dump,
                   std::string& dumpmsg);
      ...
      for (i = 0; i < analvec.size(); i++)
      {
         ...
         { // good 1st diff following bad one(s)
            ...
            { // its a slip
               ...
               {
                  results[curr].npts =
                     analvec[igood + 1].index - results[curr].index;
               ...
               results[curr].npts =
                  analvec[igood + nbad].index - results[curr].index;
               ...
               fe.step  = data[analvec[igood + nbad].index] -
                         data[analvec[igood].index];
               results.push_back(fe);
               curr++;
            }
            else
   ...
   //---------------------------------------------------------------------------------
   template <class T>
   int FirstDiffFilter<T>::analyze2(double ratlim, double siglim,
                                    bool dump, std::string& dumpmsg)
   {
      ...
      if (dump)
      {
         oss << "FirstDiff analyze2" << std::fixed << std::setprecision(3)
             << " fdlimit=" << fdlimit << " siglim=" << siglim
             << " ratlim=" << ratlim
      ...
      tstepmin =
         (noxdata ? 0 : xdata[analvec[1].index] - xdata[analvec[0].index]);
      ...
      //   std::cout << std::endl;
      //}
      //} catch(gpstk::Exception& e) { std::cout << "TST " << e.what() <<
      ...
      {                         // NB i0 changes when hit
         ...
         if (j > N + 2 && pstats.N() > 2)
         {
            ...
            tstep = xdata[analvec[i - N - 1].index] -
                    xdata[analvec[i - N - 2].index];
            if (tstep > tstepmax)
            {
               tstepmax = tstep;
            }
            ...
            if (dump)
            {
               ...
               oss << i << std::fixed << std::setprecision(3) << " "
                   << (noxdata ? T(analvec[i - N - 1].index)
                               : xdata[analvec[i - N - 1].index])
                   << " " << data[analvec[i - N - 1].index] << " " << diff
                   << " " << step << "  " << avefd << " " << sig << " " << rat
                   << "  " << pa << " " << ps << " " << pr << "  " << fa << " "
                   << fs << " " << fr << (hitslip ? " SLIP" : "")
                   << (hisig ? " SIG" : "");
   ...
   //---------------------------------------------------------------------------------
   template <class T>
   void FirstDiffFilter<T>::dump(std::ostream& os, const std::string& tag)
   {
      size_t i, j, k;
      os << "#" << tag << " FirstDiffFilter::dump() with limit " << std::fixed
         << std::setprecision(osp) << fdlimit
   ...
   template <class T> void FirstDiffFilter<T>::getStats(FilterHit<T>& fe)
   {
      ...
      {
         if ((unsigned int)(j) + i >= analvec.size() ||
             analvec[j + i].index >= k)
         {
      ...
      fe.mad = gpstk::Robust::MedianAbsoluteDeviation<T>(&fdv[0], fdv.size(),
                                                         fe.med, false);
      fe.haveStats = true;
   }
