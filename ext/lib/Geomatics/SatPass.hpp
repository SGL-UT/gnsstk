...
namespace gnsstk
{
   ...
   protected:
      ...
      void init(const RinexSatID& sat, double dt,
   ...
   public:
      ...
      // --------------------------------
         ...
         */
      double getMaxGap() const { return maxGap; }
      ...
      Epoch getLastGoodTime() const
      {
         ...
         */
      RinexSatID getSat() const { return sat; }
         ...
         while (it != subst.end())
         {
            ...
            for (jt = labelForIndex.begin(); jt != labelForIndex.end(); ++jt)
            {
               ...
               labelForIndex.insert(
                  std::map<unsigned int, std::string>::value_type(i,
                                                                  it->second));
            }
      ...
      bool hasOverlap(const SatPass& that, double *pdelt = NULL,
                      Epoch *pttb = NULL, Epoch *ptte = NULL);
      ...
      void smooth(bool smoothPR, bool smoothPH, std::string& msg,
         ...
         /**
          ...
          @throw Exception
         */
      void smoothSF(bool smoothPR, bool smoothPH, std::string& msg,
                    const int freq, double wl);
      ...
      // output -----------------------------------------------------------
         ...
         */
      std::string
      toString(std::string msg = "",
               std::string fmt =
                  "%04Y/%02m/%02d %02H:%02M:%06.3f = %04F %w %10.3g")
      {
         std::ostringstream os;
         os << msg << " " << sat << " N " << std::setw(4) << size() << " good "
            << std::setw(4) << ngood << " times "
            << printTime(getFirstTime(), fmt) << " to "
            << printTime(getLastTime(), fmt) << " obs:";
         ...
         */
      void dump(std::ostream& os, const std::string& msg1,
