...
namespace gnsstk
{
   ...
   ostream& operator<<(ostream& os, const EarthOrientation& eo)
   {
      os << " " << fixed << setw(10) << setprecision(6) << eo.xp << " "
         << setw(10) << setprecision(6) << eo.yp << " " << setw(11)
         << setprecision(7) << eo.UT1mUTC << " " << eo.convention;
      return os;
   }
   ...
   static void correctEOPOceanTides(double mjd, double& dxp, double& dyp,
                                    double& dUT);
   ...
   static void correctEarthRotationZonalTides(const double args[6], double& dUT,
                                              double& dld, double& dom);
   ...
   static void correctEarthRotationZonalTides2003(const double args[6],
      ...
      /* Corrections to UT1 and length of day (LOD) due to subdiurnal librations
         ...
         param dUT correction to UT1mUTC in seconds
         param dld correction to length of day in seconds/day */
   static void correctEarthRotationLibrations(const double args[6], double& dUT,
                                              double& dld);
   ...
   void computeFundamentalArgs(double T, double args[6])
   {
      ...
      double GMST =
         ::fmod(67310.54841 + T * (3155760000.0 + 8640184.812866 +
                                   T * T * (0.093104 + T * (-6.2e-6))),
   ...
   //---------------------------------------------------------------------------------
   void correctEOPOceanTides(double mjd, double& dxp, double& dyp,
                             double& dUT)
   {
      ...
      static const double fact[][2] = {{0.0298, 0.0200}, {0.1408, 0.0905},
                                       {0.0805, 0.0638}, {0.6002, 0.3476},
                                       {0.3025, 0.1645}, {0.1517, 0.0923}};
      ...
      static const Coeff C[] = {{2, 1, -1.94, 9.0899831, 5.18688050},
   ...
   //---------------------------------------------------------------------------------
      /* Corrections to Earth rotation due to zonal tides using USNO algorithm
         ...
         param dld correction to length of day in seconds/day
         param dom correction to yp in radians per second */
   void correctEarthRotationZonalTides(const double args[6], double& dUT,
                                       double& dld, double& dom)
   {
         // constants
      static const double TWOPI(6.283185307179586476925287);
   ...
   //---------------------------------------------------------------------------------
      /* Corrections to Earth rotation due to zonal tides using USNO IERS2003
         ...
         param dld correction to length of day in seconds/day
         param dom correction to yp in radians per second */
   void correctEarthRotationZonalTides2003(const double args[6], double& dUT,
                                           double& dld, double& dom)
   {
         // constants
      static const double TWOPI(6.283185307179586476925287);
   ...
   {
      return ::fmod(
                450160.398036           // 125.04455501 * 3600
                   + T * (-6962890.5431 // .2665
                          + T * (7.4722 + T * (0.007702 + T * (-0.00005939)))),
                ARCSEC_PER_CIRCLE) *
             ARCSEC_TO_RAD;
   ...
   {
      return ::fmod(
                335779.526232 // 93.27209062 * 3600
                   + T * (1739527262.8478 +
   ...
   double EarthOrientation::Lp(double T)
   {
      return ::fmod(1287104.793048 // 357.52910918 * 3600
                       +
                       T * (129596581.0481 +
                            T * (-0.5532 + T * (0.000136 // NB this has a minus
                                                            // sign in interp.f
                                                + T * (-0.00001149)))),
                    ARCSEC_PER_CIRCLE) *
             ARCSEC_TO_RAD;
   ...
   Matrix<double> EarthOrientation::ECEFtoInertial(const EphTime& t,
                                                   bool reduced)
   {
      try
      {
   ...
   //------------------------------------------------------------------------------
      /* Compute the locator s which gives the position of the CIO on the equator
         ...
         */
   double EarthOrientation::S(double T, double& X, double& Y,
                              IERSConvention which)
   {
      int i, j;
      double s, st[6], arg;
      ...
      static const double polycoeff[6] = {94.00e-6,     3808.35e-6, -119.94e-6,
                                          -72574.09e-6, 27.70e-6,   15.61e-6};
         // same for 2010
      ...
      { // order 1
         ...
         {
            st[1] +=
               C1_1_sincoeff2010 * ::sin(arg) + C1[i].coscoeff * ::cos(arg);
         }
         else
         {
      ...
      else // 2010
      {
         for (i = n3 - 1; i >= 0; --i)
         {
            ...
            st[3] += C32010[i].sincoeff * ::sin(arg) +
                     C32010[i].coscoeff * ::cos(arg);
         }
      }
      ...
      s = st[0] +
          (st[1] + (st[2] + (st[3] + (st[4] + st[5] * T) * T) * T) * T) * T;
      s *= ARCSEC_TO_RAD;
      s -= X * Y / 2.0;
...
#include "IERS2010CIOSeriesData.hpp"
      ...
      fa[13] = Pa(T);        // general precession in longitude
         ...
         // intermediate totals
      double xypoly[2] = {0.0, 0.0}, xylunarsolar[2] = {0.0, 0.0},
             xyplanet[2] = {0.0, 0.0};
                                               ...
                                               double& UT1mUTC)
   {
      try
      {
         ...
         int idays(int(tUT1.dMJD() - 0.5) -
                   intJulianEpoch); // days = idays+frac
         double frac(0.5 +
                     tUT1.secOfDay() / 86400.0); // fractional part of days
         if (frac > 1.0)
         {
            frac -= 1.0;
         }
   ...
   double EarthOrientation::EquationOfEquinoxes2003(EphTime t)
   {
      ...
      try
      {
         ...
         LOG(DEBUG7) << "Omega2003(T) = " << fixed << setprecision(15)
                     << farg[4];
         farg[5] = LV(T); // mean longitude of Venus
   ...
   void EarthOrientation::UT1mUTCTidalCorrections(double T, double& UT1mUT1R,
                                                  double& dlodR,
                                                  double& domegaR)
   {
...
#include "IERS1996UT1mUTCData.hpp"
   ...
   double EarthOrientation::Obliquity1996(double T)
   {
         /* double eps; // seconds of arc
            ...
            eps *= DEG_TO_RAD;
            return eps; */
      return (84381.448 + T * (-46.8150 + T * (-0.00059 + T * (0.001813)))) *
             ARCSEC_TO_RAD;
   }
   ...
   double EarthOrientation::GMST2003(EphTime t, double UT1mUTC)
   {
      try
      {
         ...
         G          = ::fmod(
            era + (0.014506 +
                   (4612.15739966 +
                    (1.39667721 + (-0.00009344 + 0.00001882 * T) * T) * T) *
                      T) *
                     ARCSEC_TO_RAD,
            TWOPI);
   ...
   double EarthOrientation::GMST2010(EphTime t, double UT1mUTC)
   {
      try
      {
         ...
         return (era +
                 (0.014506 +
                  T * (4612.156534 // NB era in radians already
                       + T * (1.3915817 +
                              T * (-0.00000044 + T * (-0.000029956 +
                                                      T * (-0.0000000368)))))) *
                    ARCSEC_TO_RAD);
      }
      catch (Exception& e)
   ...
   //---------------------------------------------------------------------------------
      /* Helper to compute the Greenwich hour angle of the true vernal equinox, or
         ...
         dpsi = nutation in longitude (counted in the ecliptic),
                        in seconds of arc. */
   double EarthOrientation::gast1996(EphTime t, double om, double eps,
                                     double dpsi, double UT1mUTC)
   {
      try
      {
   ...
   double EarthOrientation::GAST2010(EphTime t, double UT1mUTC)
   {
      try
      {
         ...
         ys = -ax * Y;
         zs = -X;
         p  = NutPreBias(0, 0) * xs + NutPreBias(0, 1) * ys +
             NutPreBias(0, 2) * zs;
         q = NutPreBias(1, 0) * xs + NutPreBias(1, 1) * ys +
             NutPreBias(1, 2) * zs;
   ...
   {
      Matrix<double> R;
      R = rotation(-epsa, 1) * rotation(-psib, 3) * rotation(phib, 1) *
          rotation(gamb, 3);
      return R;
   }
...
#include "IERS1996NutationData.hpp"
      ...
      double el =
         ::fmod((485866.733 + (715922.633 + (31.310 + 0.064 * T) * T) * T) *
                      ARCSEC_TO_RAD +
      ...
      double elp =
         ::fmod((1287099.804 + (1292581.224 + (-0.577 - 0.012 * T) * T) * T) *
                      ARCSEC_TO_RAD +
      ...
      double f =
         ::fmod((335778.877 + (295263.137 + (-13.257 + 0.011 * T) * T) * T) *
                      ARCSEC_TO_RAD +
      ...
      double d =
         ::fmod((1072261.307 + (1105601.328 + (-6.891 + 0.019 * T) * T) * T) *
                      ARCSEC_TO_RAD +
      ...
      {
            // form argument
         double arg = coeff[i].nl * el + coeff[i].nlp * elp + coeff[i].nf * f +
                      coeff[i].nd * d + coeff[i].nom * om;
...
#include "IERS2003NutationData.hpp"
      ...
      double lp(
         ::fmod(1287104.79305 // mean anomaly of the sun MHB2000 value
                   + T * (129596581.0481 +
      ...
      double f(
         ::fmod(335779.526232 // mean longitude of moon minus Omega MHB2000
                   + T * (1739527262.8478 +
                          T * (-12.7512 + T * (-0.001037 + T * (0.00000417)))),
                ARCSEC_PER_CIRCLE) *
         ARCSEC_TO_RAD);
      ...
      double d(
         ::fmod(1072260.70369 // mean elongation moon from sun MHB2000
                   + T * (1602961601.2090 +
                          T * (-6.3706 + T * (0.006593 + T * (-0.00003169)))),
                ARCSEC_PER_CIRCLE) *
         ARCSEC_TO_RAD);
      ...
      for (i = NLS - 1; i >= 0; --i)
      {
            // argument
         arg =
            ::fmod(LSCoeff[i].nl * l + LSCoeff[i].nlp * lp + LSCoeff[i].nf * f +
                      LSCoeff[i].nd * d + LSCoeff[i].nom * Om,
                   TWOPI);
         sina = ::sin(arg);
         cosa = ::cos(arg);
            // term
         deps +=
            (LSCoeff[i].ce + LSCoeff[i].cet * T) * cosa + LSCoeff[i].se * sina;
         dpsi +=
            (LSCoeff[i].sp + LSCoeff[i].spt * T) * sina + LSCoeff[i].cp * cosa;
      }
      ...
      {
            // argument
         arg  = ::fmod(PCoeff[i].nl * l + PCoeff[i].nf * f + PCoeff[i].nd * d +
                         PCoeff[i].nom * Om + PCoeff[i].nme * lme +
                         PCoeff[i].nve * lve + PCoeff[i].nea * lea +
                         PCoeff[i].nma * lma + PCoeff[i].nju * lju +
                         PCoeff[i].nsa * lsa + PCoeff[i].nur * lur +
                         PCoeff[i].nne * lne + PCoeff[i].npa * pa,
                      TWOPI);
         sina = ::sin(arg);
         cosa = ::cos(arg);
            // term
   ...
   Matrix<double> EarthOrientation::PrecessionMatrix2003(double T)
   {
      ...
      LOG(DEBUG7) << "frame bias psi = " << fixed << setprecision(15) << showpos
                  << psibias;
      LOG(DEBUG7) << "frame bias eps = " << fixed << setprecision(15) << showpos
                  << epsbias;
      LOG(DEBUG7) << "frame bias dra = " << fixed << setprecision(15) << showpos
                  << raeps0;

         // precession angles
      double psia((5038.7784 + (-1.07259 + (-0.001147) * T) * T) * T *
                  ARCSEC_TO_RAD);
      double epsa(eps0 + ((0.05127 + (-0.007726) * T) * T) * T * ARCSEC_TO_RAD);
      double chia((10.5526 + (-2.38064 + (-0.001125) * T) * T) * T *
                  ARCSEC_TO_RAD);
   ...
   Matrix<double> EarthOrientation::ECEFtoInertial1996(EphTime t, double xp,
                                                       double yp,
                                                       double UT1mUTC,
                                                       bool reduced)
   {
   ...
   //---------------------------------------------------------------------------------
      /* Generate the full transformation matrix (3x3 rotation) relating the ECEF
         ...
         (arcsecs), and UT1-UTC (sec) (xp,yp and UT1-UTC are just as found in the
         IERS bulletin; see class EarthOrientation). */
   Matrix<double> EarthOrientation::ECEFtoInertial2003(EphTime t, double xp,
                                                       double yp,
                                                       double UT1mUTC)
   {
      try
   ...
   //---------------------------------------------------------------------------------
      /* Generate the full transformation matrix (3x3 rotation) relating the ECEF
         ...
         (arcsecs), and UT1-UTC (sec) (xp,yp and UT1-UTC are just as found in the
         IERS bulletin; see class EarthOrientation). */
   Matrix<double> EarthOrientation::ECEFtoInertial2010(EphTime t, double xp,
                                                       double yp,
                                                       double UT1mUTC)
   {
      try
      {
         ...
         LOG(DEBUG7) << "s\" = " << fixed << setprecision(15)
                     << s / ARCSEC_TO_RAD;
